...
public class Condition_recommend_gender implements BAction {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//System.out.println("gender: " + gender);					//ajax값 넘어오는지 확인
		//request.setAttribute("gender", gender);
		request.setAttribute("dto_gender", dao.gender_res1(dto));
		//System.out.println("1.성별선택시 NO확인: "+request.getAttribute("dto_gender"));
		
		Gson gson = new Gson();
		JsonArray array = new JsonArray();
		ArrayList<KeywordDto> list = dao.gender_res1(dto);
		for(int i=0; i<list.size(); i++) {
			JsonObject obj = new JsonObject();
				...
				//System.out.println("확인111: " +list.get(i).getPic());
				array.add(obj);
